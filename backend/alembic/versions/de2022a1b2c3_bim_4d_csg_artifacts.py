"""bim 4d exact csg artifacts"""
from alembic import op
import sqlalchemy as sa

revision = "de2022a1b2c3"
down_revision = "de2021a1b2c3"
branch_labels = None
depends_on = None
...
def upgrade():
    op.create_table(
        ...
        sa.Column("artifact_revision", sa.String(100), nullable=False),
        ...
        sa.Column("source_volume", sa.Float(), nullable=False),
        sa.Column("partition_volume", sa.Float(), nullable=False),
        sa.Column("conservation_delta", sa.Float(), nullable=False),
        sa.Column("created_by", sa.Integer(), sa.ForeignKey("usuarios.id", ondelete="SET NULL")),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.UniqueConstraint("partition_spec_id", "artifact_revision", name="uq_bim_4d_partition_csg_spec_revision"),
    )
    for column in ("empresa_id", "proyecto_id", "bim_model_version_id", "bim_element_id", "partition_spec_id"):
        op.create_index(f"ix_bim_4d_partition_csg_artifacts_{column}", "bim_4d_partition_csg_artifacts", [column])


def downgrade():
    op.drop_table("bim_4d_partition_csg_artifacts")
