...
from app.schemas.bim_operations_transition import BimOperationsTransitionCreate, BimOperationsTransitionDecision, BimOperationsTransitionResponse
...
@router.get("/projects/{project_id}/schedules-of-values", response_model=list[BimCostSovResponse])
def list_project_bim_sovs(project_id: int, empresa_id: Optional[int] = None, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_active_user)):
    project = _resolve_project(db, project_id, current_user, empresa_id); _require_bim_access(db, project, current_user, "bim.view")
    return list_sovs(db, project_id=project.id, company_id=project.empresa_id)


@router.post("/projects/{project_id}/schedules-of-values", response_model=BimCostSovResponse, status_code=status.HTTP_201_CREATED)
def create_project_bim_sov(project_id: int, payload: BimCostSovCreate, empresa_id: Optional[int] = None, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_active_user)):
    project = _resolve_project(db, project_id, current_user, empresa_id); _require_bim_access(db, project, current_user, "bim.review")
    return create_sov(db, project_id=project.id, company_id=project.empresa_id, user_id=current_user.id, payload=payload)


@router.post("/projects/{project_id}/schedules-of-values/{sov_id}/decision", response_model=BimCostSovResponse)
...
@router.get("/projects/{project_id}/commissioning", response_model=BimCommissioningRegistryResponse)
def get_project_bim_commissioning(project_id: int, empresa_id: Optional[int] = None, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_active_user)):
    project = _resolve_project(db, project_id, current_user, empresa_id); _require_bim_access(db, project, current_user, "bim.view")
    return get_commissioning_registry(db, project_id=project.id, company_id=project.empresa_id)


@router.post("/projects/{project_id}/commissioning/systems", response_model=BimCommissioningSystemResponse, status_code=status.HTTP_201_CREATED)
def create_project_bim_commissioning_system(project_id: int, payload: BimCommissioningSystemCreate, empresa_id: Optional[int] = None, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_active_user)):
    project = _resolve_project(db, project_id, current_user, empresa_id); _require_bim_access(db, project, current_user, "bim.coordinate")
    return create_commissioning_system(db, project_id=project.id, company_id=project.empresa_id, user_id=current_user.id, payload=payload)


@router.post("/projects/{project_id}/commissioning/assets", response_model=BimCommissioningAssetResponse, status_code=status.HTTP_201_CREATED)
...
@router.post("/projects/{project_id}/handover-dossiers/{dossier_id}/decision", response_model=BimHandoverDossierResponse)
...
@router.post("/projects/{project_id}/operations-transitions/{transition_id}/decision", response_model=BimOperationsTransitionResponse)
def decide_project_bim_operations_transition(project_id: int, transition_id: int, payload: BimOperationsTransitionDecision, empresa_id: Optional[int] = None, db: Session = Depends(get_db), current_user: Usuario = Depends(get_current_active_user)):
    project = _resolve_project(db, project_id, current_user, empresa_id); _require_bim_access(db, project, current_user, "bim.coordinate")
    return decide_operations_transition(db, transition_id=transition_id, project_id=project.id, company_id=project.empresa_id, user_id=current_user.id, payload=payload)
