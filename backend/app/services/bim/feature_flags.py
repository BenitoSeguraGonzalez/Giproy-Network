...
class BimFeatureAccess:
    ...
    commercial_entitlement_required: bool
    commercial_entitled: bool
    commercial_entitlement_source: str | None
...
def resolve_bim_feature_access(
    *,
    ...
    commercial_entitlement_source = None
    ...
    if db is not None and system_bim_settings_table_ready(db):
        ...
        commercial_entitlement_required = (
            db.query(Licencia.id)
            .filter(Licencia.activo == True)
            .first()
            is not None
        )
        if commercial_entitlement_required and company_id is not None:
            commercial_state = commercial_capabilities_service.resolve_company_capabilities(
                db,
                company_id,
            )
            commercial_entitled = bool(
                commercial_state.get("access_mode") == "full"
                and commercial_state.get("capabilities", {}).get("bim")
            )
            commercial_entitlement_source = (
                commercial_state.get("bim_entitlement", {}).get("source")
            )
        elif commercial_entitlement_required:
            commercial_entitled = False
    ...
    rollout_enabled = environment_enabled and role_match and (
        not scoped
        or company_match
        or user_match
        or (superadmin_only and role_match and not scoped)
    )
    enabled = rollout_enabled and commercial_entitled
    ...
    return BimFeatureAccess(
        ...
        commercial_entitled=commercial_entitled,
        commercial_entitlement_source=commercial_entitlement_source,
    )
