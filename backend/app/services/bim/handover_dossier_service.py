...
def _governed_sources(db, *, project_id, company_id):
    as_built = db.query(BimAsBuiltAcceptance).filter(
    ...
    punch = db.query(BimPunchClosure).filter(
    ...
    documents = db.query(BimCdeDocument, BimCdeDocumentRevision).join(
    ...
    ).order_by(BimCdeDocument.document_code, BimCdeDocumentRevision.id).all()
    if not documents:
