...
try {
    ...
    await page.goto(`${baseUrl}/bim-cde-acl-harness.html`, { waitUntil: 'domcontentloaded' });
    await page.locator('[data-bim-cde-acl]').waitFor(); await page.getByLabel('Usuario ACL CDE').selectOption('8');
    await page.getByLabel('Revisar ACL CDE').check(); await page.getByLabel('Guardar ACL CDE').click();
    await page.locator('[data-bim-cde-acl-list]').getByText('Revisor BIM', { exact: true }).waitFor();
    assert.match(await page.locator('[data-bim-cde-acl-list]').innerText(), /ver.*descargar.*revisar/, 'La jerarquia de permisos queda visible');
    await page.getByRole('button', { name: 'Revocar' }).click(); await page.getByRole('button', { name: 'Activar' }).waitFor();
    ...
    await page.screenshot({ path: `${process.env.TEMP || '.'}/giproy-bim-cde-acl-1920x1080.png`, fullPage: true });
    await context.close(); console.log('validate-bim-cde-acl-dom: ok');
