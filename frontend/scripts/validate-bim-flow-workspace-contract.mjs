...
const fragmentsWorker = read('src/components/bim/ifcFragmentsWorker.js');
...
for (const token of ['IfcImporter', 'importer.process', 'web-ifc.wasm?url']) {
  if (!fragmentsWorker.includes(token)) throw new Error(`contrato IFC/Fragments incompleto: falta ${token}`);
}
for (const token of ['source_ifc', "new Worker(new URL('./ifcFragmentsWorker.js', import.meta.url)"]) {
