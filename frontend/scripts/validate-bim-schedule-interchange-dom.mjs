...
try {
    ...
    await page.getByRole('button', { name: 'Primavera P6 XER' }).click();
    await page.getByLabel('Archivo de planificacion').setInputFiles({ name: 'gantt-r3.xer', mimeType: 'application/octet-stream', buffer: Buffer.from('ERMHDR\t24.12') });
    ...
    await page.getByRole('button', { name: 'Exportar .xer' }).click();
    const download = await downloadPromise;
    assert.equal(download.suggestedFilename(), 'giproy-bim-schedule-p6.xer', 'La exportacion XER usa nombre estable');
