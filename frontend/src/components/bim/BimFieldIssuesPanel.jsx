...
const BimFieldIssuesPanel = ({ projectId, versionId, empresaId, viewerState, onOpenIssue, api = bimModelsApi, onOpenAttachment }) => {
    ...
    return (
        <section className="bim-adaptive-master-detail grid h-full min-h-0 overflow-hidden rounded-lg border border-zinc-200 bg-white" data-bim-field-issues>
            ...
            <div className="min-h-0 overflow-y-auto" data-bim-field-issue-detail>
                ...
                {message ? <p className="sticky bottom-2 ml-auto mr-2 w-fit rounded-md border border-zinc-200 bg-white px-3 py-2 text-[10px] font-medium text-zinc-700 shadow-sm" role="status">{message}</p> : null}
            </div>
        </section>
    );
};
