...
export default function BimFlowWorkspace({ project, access, onNavigateTarget, workspaceOverride = null, domainStateOverride = null }) {
  ...
  const stagePanel = {
    model: <div className="grid min-h-0 gap-4 p-4 xl:grid-cols-[minmax(0,1fr)_minmax(20rem,28rem)]"><div className="grid min-h-0 gap-4"><BimImportJobsPanel projectId={project?.id} empresaId={empresaId} onImportReady={refresh} /><BimElementExplorerPanel elements={effectiveWorkspace?.elements || []} selectedElementId={selectedElement?.id || selectedElement?.global_id} onSelectElement={setSelectedElement} /><BimCanvasViewer elements={effectiveWorkspace?.elements || []} ready={Boolean(effectiveWorkspace?.elements?.length)} error={error} selectedElement={selectedElement} onSelectElement={setSelectedElement} activeVersionLabel={effectiveWorkspace?.active_version_label} /></div><div className="grid min-h-0 gap-4"><BimVersionSelector models={effectiveWorkspace?.models || []} activeVersionId={activeVersionId || effectiveWorkspace?.active_version_id} onSelectVersion={setActiveVersionId} /><BimSavedViewsPanel views={viewStates} onCreate={async (payload) => { await bimViewStatesApi.createByProject(project?.id, { ...payload, version_id: activeVersionId || effectiveWorkspace?.active_version_id || null }, empresaId); refresh(); }} /></div></div>,
