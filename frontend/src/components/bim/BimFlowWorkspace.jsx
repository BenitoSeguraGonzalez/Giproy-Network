...
export default function BimFlowWorkspace({ project, access, onNavigateTarget, workspaceOverride = null, domainStateOverride = null }) {
  ...
  const modelStage = <div className="grid min-h-0 min-w-0 flex-1 gap-3 p-3 xl:grid-cols-[minmax(0,1fr)_22rem]"><div className="min-h-0 min-w-0 overflow-hidden"><BimCanvasViewer elements={effectiveWorkspace?.elements || []} ready={Boolean(effectiveWorkspace?.elements?.length)} error={error} selectedElement={selectedElement} onSelectElement={setSelectedElement} activeVersionLabel={effectiveWorkspace?.active_version_label} /></div><aside className="min-h-0 min-w-0 space-y-3 overflow-y-auto"><BimElementExplorerPanel elements={effectiveWorkspace?.elements || []} selectedElementId={selectedElement?.id || selectedElement?.global_id} onSelectElement={setSelectedElement} /><BimVersionSelector models={effectiveWorkspace?.models || []} activeVersionId={activeVersionId || effectiveWorkspace?.active_version_id} onSelectVersion={setActiveVersionId} /><BimImportJobsPanel projectId={project?.id} empresaId={empresaId} onImportReady={refresh} /><BimSavedViewsPanel views={viewStates} onCreate={async (payload) => { await bimViewStatesApi.createByProject(project?.id, { ...payload, version_id: activeVersionId || effectiveWorkspace?.active_version_id || null }, empresaId); refresh(); }} /></aside></div>;
  const stagePanel = {
    model: modelStage,
